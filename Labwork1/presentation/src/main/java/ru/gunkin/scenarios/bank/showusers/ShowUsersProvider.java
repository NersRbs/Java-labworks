...
public class ShowUsersProvider implements ScenarioProvider {
    private final CurrentService currentService;
    private final BankService bankService;

    public ShowUsersProvider(CurrentService currentService, BankService bankService) {
    ...
    public Scenario tryGetScenario() {
        if (currentService.getState() instanceof AdminState) {
            return new ShowUsersScenario(bankService);
        }
        return null;
    }
}
