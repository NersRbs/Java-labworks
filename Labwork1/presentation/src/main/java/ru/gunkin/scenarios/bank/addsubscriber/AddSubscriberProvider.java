...
public class AddSubscriberProvider implements ScenarioProvider {
    private final CurrentService currentService;
    private final BankService bankService;

    public AddSubscriberProvider(CurrentService currentService, BankService bankService) {
    ...
    public Scenario tryGetScenario() {
        if (currentService.getState() instanceof AdminState)
            return new AddSubscriberScenario(bankService);
        return null;
    }
}
